...
def _readTxt():
    ...
    for line in dataTxt:
        List = line.split(",")
        vars()[List[0]] = Product(List[1], List[2], int(List[3]))
        DATA[List[0]] = vars()[List[0]]
    dataTxt.close()
...
def _writeTxt():
    ...
    dataTxt.write("") #Limpando arquivo .txt
    dataTxt.close()
    ...
    for key,value in DATA.items():
        if firstLine:
            dataTxt.write(f'{key},{value.name},'
                          f'{value.model},{value.qt}, ')
            firstLine = False
        else:
            dataTxt.write(f'\n{key},{value.name},'
                          f'{value.model},{value.qt}, ')
